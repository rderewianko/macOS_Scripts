...
########################################################################
#           Delete decomissioned hardware computer records             #
...
if [[ -f "/var/tmp/Decomissioned_Computers.txt" ]]; then
  echo -e "Finalizing csv file...\n"

  #Use paste to join all data into a final csv file
  paste -s -d'\n' "/var/tmp/Decomissioned_Computers.txt" >> "/var/tmp/Decomissioned_Computers.csv"

  #Rename the final csv file with the current date
  mv "/var/tmp/Decomissioned_Computers.csv" "/var/tmp/Decomissioned_Computers_${date}.csv"
else
...
if [ -s /var/tmp/Decomissioned_Computers_${date}.csv ]; then
  echo "Sending email with details of deleted records"
  echo "Attached are the decomissioned computers deleted from Jamf Pro (Production)" | mailx -r jamfpro.notifications@bauerservices.co.uk -s "Jamf Pro (Production) decomissioned computers" -a "/var/tmp/Decomissioned_Computers_${date}.csv" -- 5a6216ec.bauerahead.com@emea.teams.ms
else
...
while read ComputerID; do
  ...
  if [[ "$computerDepartment" == "$decomDept" ]]; then
          echo "$computerName has been decomissioned"
          echo "Deleting computer record for $computerName..."
          #Export the computer details to a file for reference
          curl -H "Accept: text/xml" -sfku "${apiuser}:${apipass}" "${jssurl}/JSSResource/computers/id/$ComputerID" 2>/dev/null | xmllint --format --xpath  "/computer/general/name | /computer/general/serial_number | /computer/location/department" - | awk -F'>|<' '{print $3,$7,$11}' >> /var/tmp/Decomissioned_Computers.txt
...
rm -f /var/tmp/Decomissioned_Computers*
