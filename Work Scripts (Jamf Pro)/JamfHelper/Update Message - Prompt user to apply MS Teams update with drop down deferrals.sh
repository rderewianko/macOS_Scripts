...
########################################################################
#         Microsoft Teams update with JamfPro script variables         #
...
{
HELPER=$(
/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType utility -icon /System/Library/CoreServices/Installer.app/Contents/Resources/Installer.icns -title "Message from Bauer IT" -heading "An important update for Microsoft Teams is waiting to be installed" -alignHeading center -description "The update brings new features, bug fixes and automatic update functionality.

You may choose to install the update now or select one of the deferral times if you currently require the use of Teams. After the deferral time lapses the update will be automatically installed and Teams will be closed during the process.

Please make sure you do not need to use Teams before the update starts!" -lockHUD -showDelayOptions "$deferralOption1, $deferralOption2, $deferralOption3, $deferralOption4"  -button1 "Select"
...
HELPER_CONFIRM=$(
/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType utility -icon /System/Library/CoreServices/Installer.app/Contents/Resources/Installer.icns -title "Message from Bauer IT" -heading "    An important update for Microsoft Teams is now ready to be installed     " -description "The update brings new features, bug fixes and automatic update functionality.

Microsoft Teams will be closed automatically during the update process." -lockHUD -timeout 21600 -button1 "Install" -defaultButton 1
)
}
...
local H=$((T/60/60%24));
local M=$((T/60%60));
timeChosenHuman=$(printf '%s' "An update for Microsoft Teams will be installed in: "; [[ $D > 0 ]] && printf '%d days ' $D; [[ $H -eq 1 ]] && printf '%d hour' $H; [[ $H -ge 2 ]] && printf '%d hours' $H; [[ $M > 0 ]] && printf '%d minutes' $M; [[ $D > 0 || $H > 0 || $M > 0 ]] )
#Show a message via Jamf Helper that the update will be installed after the deferral time chosen
HELPER_DEFERRAL_CONFIRM=$(
/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType utility -icon /Library/Application\ Support/JAMF/bin/Management\ Action.app/Contents/Resources/Self\ Service.icns -title "Message from Bauer IT" -heading "    $timeChosenHuman      " -description "If you would like to install the update sooner please open Self Service and navigate to the Featured section." -timeout 10  -button1 "Ok" -defaultButton 1 &
)
}
...
/Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType utility -icon /Library/Application\ Support/JAMF/bin/Management\ Action.app/Contents/Resources/Self\ Service.icns -title "Message from Bauer IT" -heading "    Update Complete     " -description "Microsoft Teams update successfully installed." -button1 "Ok" -defaultButton "1"
jamfmsg3
}
...
if [ "$LoggedInUser" == "" ]; then
    echo "No logged in user, apply MS Teams update."
