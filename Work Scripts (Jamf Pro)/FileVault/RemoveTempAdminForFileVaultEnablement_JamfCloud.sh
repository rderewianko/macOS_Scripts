...
# Loop through each account found and remove them from the admin group (excluding root, admin and jamfcloudadmin)
for user in $adminUsers; do
    if [[ "$user" != "root" && "$user" != "admin" && "$user" != "jamfcloudadmin" ]]; then
