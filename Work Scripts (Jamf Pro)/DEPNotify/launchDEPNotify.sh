...
########################################################################
#            Launch DEP Notify and complete Mac setup process          #
...
hostName=$(scutil --get HostName)	# the hostname for this Mac
...
function DEPNotifyGraphics ()
{
  ...
  echo "Getting Company logo for DEP Notify"
  curl -s --url https://www.companyname.com/fileadmin/site/img/logo_header.png > /var/tmp/company_logo.png
}
...
sudo /usr/sbin/scutil --set ComputerName "${wksComputerName}"
sudo /usr/sbin/scutil --set LocalHostName "${wksComputerName}"
sudo /usr/sbin/scutil --set HostName "${wksComputerName}"
...
#First check if we can get to AD
  domainPing=$(ping -c1 -W5 -q yourdomain 2>/dev/null | head -n1 | sed 's/.*(\(.*\))/\1/;s/:.*//')
  if [[ "$domainPing" == "" ]]; then
...
{
#Check if the custom trigger to cancel failed commands has already been added to the startup script - the startup script gets overwirtten during a jamf manage.
...
if [ -f "/var/tmp/com.depnotify.registration.done" ]; then
  ...
  # Disable Jamf Check-In so nothing conflicts or stops DEPNotify from calling JAMFpro Policies
  ...
  until [ -f /var/tmp/com.depnotify.registration.done ]
  do
  ...
  /bin/echo "Command: Image: /var/tmp/company_logo.png" >> /var/tmp/depnotify.log
  #/bin/echo "Command: Image: /System/Library/CoreServices/Applications/Directory\ Utility.app/Contents/Resources/DirectoryUtility.icns" >> /var/tmp/depnotify.log
  ...
  #Upodate computer name
  /bin/echo "Status: Saving Computer Name - $wksComputerName....." >> /var/tmp/depnotify.log
  updateComputerName
  #Bind to AD and update JamfPro - Checks if AD is available in the function
  /bin/echo "Status: Joining $wksComputerName to the Company domain....." >> /var/tmp/depnotify.log
  rebindtoAD
  # Send selected department to JamfPro
