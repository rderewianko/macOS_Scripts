...
########################################################################
#  Set OneDrive to the Production update ring for the logged in user   #
...
echo "Checking OneDrive update ring settings..."
 if [[ "$OneDriveTier" == "" ]] || [[ "$OneDriveTier" == "Production" ]] ; then
  echo "OneDrive already set to Production update ring, nothing to do"
  exit 0
else
  echo "Setting OneDrive to the Production update ring"
  killall OneDrive
  su -l "$LoggedInUser" -c "Defaults write com.microsoft.OneDriveUpdater Tier Production"
fi
}
